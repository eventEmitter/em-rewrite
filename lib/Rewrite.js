	...
	module.exports = new Class( {
		...
		, addRule: function( from, to, external, fromOptions ){
			...
			// ne http 301, 302 or nothing
			if ( external && external !== 301 && external !== 302 ) external = 302;
		...
		, _rewrite: function( rule, request, response, next ){
			...
			if ( rule.str ){
				// string rule
				if ( rule.external ) response.send( rule.external, { Location: rule.to.to + this._getQuerystring( request ) } );
				else {
				...
				if ( regResult ){
					if ( rule.map ){
						...
						for( var i = 1, l = regResult.length; i < l; i++ ){
							to = to.replace( new RegExp( "([^\\\\])\\$" + i, "gi" ), "$1" + regResult[ i ] );
						}
					}
				}


				if ( rule.external ) response.send( rule.external, { Location: to + this._getQuerystring( request ) } );
				else { 
					request.pathname = to;
					if ( rule.to.method ) request.method = rule.to.method;
					next();
			...
			else throw new Error( "The rule «"+rule.from+"» is invalid!" ).setName( "InvalidRewriteRuleException" );
		}
